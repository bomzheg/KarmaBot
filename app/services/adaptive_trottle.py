...
class AdaptiveThrottle:
    ...
    def check_time_throttle(self, rate: int, key: str, chat_id: int, user_id: int, target_user_id: int):
        logger.debug(
        ...
        try:
            ...
            logger.debug("there is no delta", )
            return True
        finally:
            bucket[key] = now

        self.set_bucket(chat_id, user_id, target_user_id, bucket)
        logger.debug("delta is {delta}", delta=now - last)
        return now - last > rate
