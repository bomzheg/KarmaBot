...
class ChatRepo:
    ...
    async def get_top_karma_list(self, chat: Chat, limit: int = 15) -> list[TopResultEntry]:
        ...
        users_karmas = await chat.user_karma.order_by(*karma_filters).limit(limit).prefetch_related("user").all()
        rez = []
    ...
    async def get_neighbours(self, user: User, chat: Chat) -> tuple[UserKarma, UserKarma, UserKarma]:
        prev_id, next_id = await self.get_neighbours_id(chat.chat_id, user.id)
        uk = await chat.user_karma.filter(
